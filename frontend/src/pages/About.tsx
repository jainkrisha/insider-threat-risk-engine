...
const TIMELINE = [
  {
    ...
    body: `The CERT Insider Threat Dataset r4.2 contains 17 months of synthetic but realistic
      enterprise activity across ~4,000 employees â€” logon events, file transfers, device connections,
      emails, and web browsing logs â€” with a ground-truth label of 70 malicious insiders.`,
  },
  {
    icon: <Layers className="w-5 h-5 text-indigo-400" />,
    title: 'Feature Engineering',
    body: `For each user-day, Vigil computes 15 behavioural features: logon counts,
      ...
      own historical baseline â€” not a generic company average.`,
  },
  {
    ...
    body: `Isolation Forest (60% weight) handles novel, unseen anomalies without labelled data.
      ...
      high-risk days â€” catching both spikes and sustained slow-burn activity.`,
  },
  {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 space-y-16 animate-fade-in">
      ...
      <section className="space-y-4">
        ...
        <h1 className="text-4xl font-bold">How Vigil detects insider threats</h1>
        <p className="text-[#26201b] text-lg leading-relaxed">
          Vigil is a research prototype built to solve the{' '}
          <span className="text-[#26201b] font-medium">Privileged Access Misuse & Insider Threat Detection</span>{' '}
        ...
        {TIMELINE.map((step, i) => (
          <div key={step.title} className="flex gap-5">
            <div className="flex flex-col items-center">
              ...
              {i < TIMELINE.length - 1 && (
                <div className="w-px flex-1 bg-[#dfd2bc]/60 mt-2" />
              )}
            </div>
            <GlassCard className="flex-1 mb-4 space-y-2">
              <h2 className="font-semibold text-lg">{step.title}</h2>
              <p className="text-sm text-[#26201b] leading-relaxed whitespace-pre-line">{step.body}</p>
            </GlassCard>
          </div>
        ))}
      </section>

      <GlassCard className="glass-hover border-[#496b52]/20 space-y-3">
        <h3 className="font-semibold text-indigo-300">âš ï¸ Prototype Disclaimer</h3>
        <p className="text-sm text-[#26201b] leading-relaxed">
          Vigil is a hackathon prototype trained on a synthetic dataset. The F1 score of 0.635
